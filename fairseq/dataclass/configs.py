...
class CommonConfig(FairseqDataclass):
    ...
    use_wandb: bool = field(
        default=False,
        metadata={"help": "enable Weights and Biases logging"},
    )
    ...
    wandb_entity: Optional[str] = field(
        default=None,
        metadata={"help": "Weights and Biases entity (team) to use for logging"},
    )
    ...
    ema_decay: float = field(
        default=0.999,
        metadata={"help": "decay factor for exponential moving average of model parameters (0 = disabled)"},
    )
    ema_device: str = field(
        default="cuda",
        metadata={"help": "device to store EMA model (cpu or cuda)"},
    )
    ema_fp32: bool = field(
        default=True,
        metadata={"help": "store EMA model in FP32 precision"},
    )
...
class GenerationConfig(FairseqDataclass):
    beam: int = field(
        default=10, metadata={"help": "beam size"},
    )
    nbest: int = field(
        default=10, metadata={"help": "number of hypotheses to output"},
    )
