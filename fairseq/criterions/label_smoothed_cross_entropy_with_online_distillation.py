...
class LabelSmoothedCrossEntropyCriterionWithOnlineDistillationConfig(
    ...
    reaction_aware_online_distillation: bool = field(
...
class LabelSmoothedCrossEntropyCriterionWithOnlineDistillation(
    ...
    def __init__(self, task, sentence_avg, label_smoothing, online_distillation_weight, selective_online_distillation,
                 reaction_aware_online_distillation,
        ...
        self.reaction_aware_online_distillation = reaction_aware_online_distillation
        ...
        logger.debug(f"reaction_aware_online_distillation: {self.reaction_aware_online_distillation}")
        ...
        if task.MNMT_mode == "O2M" and reaction_aware_online_distillation:
            language_aware_target_mask = {}
            for lang in task.target_langs:
                # 1. loading language-specific dicts
                LS_dict = TranslationMultiSimpleEpochTask.load_dictionary(f"{task.args.data}/RS_dict.{lang}.txt")
    ...
    def compute_online_distillation_loss(self, student, student_output, teacher, teacher_output, sample, reduce=True,
        ...
        if self.reaction_aware_online_distillation:
