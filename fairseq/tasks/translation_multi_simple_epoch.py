...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        self.source_langs = [d.split("-")[0] for d in self.reaction_pairs]
        self.target_langs = [d.split("-")[1] for d in self.reaction_pairs]
        self.check_dicts(self.dicts, self.source_langs, self.target_langs)

        self.sampling_method = SamplingMethod.build_sampler(args, self)
        self.data_manager = MultilingualDatasetManager.setup_data_manager(
            args, self.reaction_pairs, langs, dicts, self.sampling_method, pure_batch=self.pure_batch
        )
    ...
    def train_step_with_online_distillation(self, sample, model, criterion, optimizer, update_num, ignore_grad, epoch):
        """
        This code is shit: I have put the RS_best_models to this task (TranslationMultiSimpleEpochTask).
        """
        # test code:
        criterion.dicts = self.dicts

        RS_best_models = self.RS_best_models
        ...
        need_online_distillation = self.whether_to_online_distillation(RS_best_models, batch_lang, epoch)
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss, sample_size, logging_output = criterion(model,
                                                              sample,
                                                              RS_best_models[batch_lang]["model"],
    ...
    def whether_to_online_distillation(self, RS_best_models, batch_lang, epoch):
        """
        ...
        if epoch > self.args.online_distillation_warmup_epoch \
                and epoch - RS_best_models[batch_lang]["epoch"] > 1 \
                and RS_best_models[batch_lang]["model"] is not None:
            flag = True
        else:
            flag = False
