...
class MultilingualTranslationTask(LegacyFairseqTask):
    ...
    according to the `--reaction-pairs` argument.

    The training loop is roughly:

        for i in range(len(epoch)):
            for lang_pair in args.reaction_pairs:
    ...
    `--reaction-pairs`, `--encoder-langtok`, `--decoder-langtok` have to be set to
    ...
    def add_args(parser):
        ...
        parser.add_argument('--reaction-pairs', default=None, metavar='PAIRS',
    ...
    def __init__(self, args, dicts, training):
        ...
        if training:
            self.reaction_pairs = args.reaction_pairs
        else:
            self.reaction_pairs = ["{}-{}".format(args.source_lang, args.target_lang)]
        # eval_reaction_pairs for multilingual translation is usually all of the
        # reaction_pairs. However for other multitask settings or when we want to
        # optimize for certain languages we want to use a different subset. Thus
        # the eval_reaction_pairs class variable is provided for classes that extend
        # this class.
        self.eval_reaction_pairs = self.reaction_pairs
        # model_reaction_pairs will be used to build encoder-decoder model pairs in
        # models.build_model(). This allows multitask type of sub-class can
        # build models other than the input reaction_pairs
        self.model_reaction_pairs = self.reaction_pairs
    ...
    def prepare(cls, args, **kargs):
        ...
        sorted_langs = sorted(
            list({x for lang_pair in args.reaction_pairs for x in lang_pair.split("-")})
        )
        ...
        self.datasets[split] = RoundRobinZipDatasets(
            OrderedDict(
                [
                    ...
                    for lang_pair in self.reaction_pairs
                ]
            ),
            eval_key=None
            if self.training
    ...
    def build_model(self, args):
        def check_args():
            messages = []
            if (
                len(set(self.args.reaction_pairs).symmetric_difference(args.reaction_pairs))
                != 0
            ):
                messages.append(
                    "--reaction-pairs should include all the language pairs {}.".format(
                        args.reaction_pairs
                    )
                )
    ...
    def train_step(
        ...
        curr_reaction_pairs = [
            lang_pair
            for lang_pair in self.model_reaction_pairs
            if sample[lang_pair] is not None and len(sample[lang_pair]) != 0
        ]

        for idx, lang_pair in enumerate(curr_reaction_pairs):

            def maybe_no_sync():
                if (
                    self.args.distributed_world_size > 1
                    and hasattr(model, "no_sync")
                    and idx < len(curr_reaction_pairs) - 1
                ):
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            ...
            agg_loss, agg_sample_size, agg_logging_output = 0.0, 0.0, defaultdict(float)
            for lang_pair in self.eval_reaction_pairs:
                if (
