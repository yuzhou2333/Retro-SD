...
class SemisupervisedTranslationTask(MultilingualTranslationTask):
    ...
    `--target-lang`, instead of `--reaction-pairs`.
    """
    ...
    def __init__(self, args, dicts, training):
        ...
        if self.lambda_denoising > 0.0 or self.lambda_denoising_steps is not None:
            denoising_reaction_pairs = [
                "%s-%s" % (tgt, tgt)
                for tgt in {lang_pair.split("-")[1] for lang_pair in args.reaction_pairs}
            ]
            self.model_reaction_pairs = self.model_reaction_pairs + denoising_reaction_pairs
