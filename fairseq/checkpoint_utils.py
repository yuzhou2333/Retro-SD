...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, RS_valid_loss=None):
    ...
    if RS_valid_loss is not None:
        extra_state = {"train_iterator": epoch_itr.state_dict(), "val_loss": val_loss, "RS_valid_loss": RS_valid_loss}
    else:
        extra_state = {"train_iterator": epoch_itr.state_dict(), "val_loss": val_loss}
    if hasattr(save_checkpoint, "best"):
        extra_state.update({"best": save_checkpoint.best})

    # Add EMA state dict to extra_state if EMA is enabled
    if hasattr(trainer, 'get_ema_state_dict') and trainer.get_ema_state_dict() is not None:
        extra_state["ema_state_dict"] = trainer.get_ema_state_dict()
...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    # Load EMA state dict if present in checkpoint
    if extra_state is not None and "ema_state_dict" in extra_state:
        if hasattr(trainer, 'load_ema_state_dict'):
            trainer.load_ema_state_dict(extra_state["ema_state_dict"])
