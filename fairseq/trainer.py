...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self._ema_decay = getattr(cfg.common, "ema_decay", 0.0)
        self._ema_device = "cpu" if getattr(cfg.common, "ema_device", "cpu") == "cpu" else self.device
        self._ema_fp32 = getattr(cfg.common, "ema_fp32", False)
        if self._ema_decay > 0:
            logger.info(f"EMA enabled with decay={self._ema_decay}, device={self._ema_device}, fp32={self._ema_fp32}")
    ...
    def _build_optimizer(self):
        ...
        elif self.cfg.common.fp16 or self.cfg.common.bf16 or self.cfg.common.amp: # this branch
    ...
    def train_step(self, samples, raise_oom=False, epoch=-1, step=0):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    if getattr(self.cfg.task, "online_distillation_MNMT", False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                ...
                # logger.debug("活到了这个：6")
                self.task.optimizer_step(
                    self.optimizer, model=self.model, update_num=self.get_num_updates()
                )

                # Added by Eachan: EMA online-distillation
                if self.cfg.task.EMA:
                    EMA_alpha = 0.999
                    model_state_dict = self.model.state_dict()
                    EMA_model_state_dict = self.EMA_model.state_dict()
                    for name in EMA_model_state_dict:
                        EMA_model_state_dict[name] = EMA_alpha * EMA_model_state_dict[name] \
                                                     + (1 - EMA_alpha) * model_state_dict[name]
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            if getattr(cfg.task, "RS_epoch", None) and (sample is not None and len(sample) > 0):
                ...
                # 1. get lang of this batch (由于目前的做法是pad符号加在了句首，所以我得先跳过pad符号。但是我也不知道pad符号为多少，只知道<5)
                ...
                assert len(set(langs)) == 1, \
                    "this valid batch contains more than on language types."
                batch_lang = cfg.model.id2lang[langs[0]]

                # 2. modify the logging_output for async:
                src_langs = set()
                tgt_langs = set()
                for lang_pair in cfg.model.reaction_pairs.split(','):
                    ...
                    logging_output[src_lang + '-' + tgt_lang + '_loss'] = torch.zeros_like(_loss)
                    logging_output[src_lang + '-' + tgt_lang + '_size'] = 0

                # 根据是O2M还是M2O
                if len(src_langs) == 1:  # O2M
                    src_lang = list(src_langs)[0]
                    logging_output[src_lang + '-' + batch_lang + '_loss'] = logging_output['loss']
                    logging_output[src_lang + '-' + batch_lang + '_size'] = logging_output['sample_size']
                elif len(tgt_langs) == 1:  # M2O
                    tgt_lang = list(tgt_langs)[0]
                    logging_output[batch_lang + '-' + tgt_lang + '_loss'] = logging_output['loss']
                    logging_output[batch_lang + '-' + tgt_lang + '_size'] = logging_output['sample_size']
                else:
                    print("Not support M2M for -RS-epoch now.")
                    raise Exception
        ...
        # gather logging outputs from all replicas
        if self.data_parallel_world_size > 1:
            # print("before aggregate:", logging_outputs)
            logging_outputs, (sample_size,) = self._aggregate_logging_outputs(
                logging_outputs,
                sample_size,
                ignore=is_dummy_batch,
            )
            # print("after aggregate:", logging_outputs)

        # log validation stats
        if self.tpu:
            logging_outputs = self._xla_markstep_and_send_to_cpu(logging_outputs)

        # added by eachan: 由于reduce的时候，一些属性会被删掉，所以我先保留一下，后面加进去。
        if getattr(cfg.task, "RS_epoch", None) and (sample is not None and len(sample) > 0):
            LS_valid_logging_output = {}
            for lang_pair in cfg.model.reaction_pairs.split(','):
                src_lang, tgt_lang = lang_pair.split('-')
                if len(src_langs) == 1:  # O2M
                    LS_valid_logging_output[src_lang + '-' + tgt_lang + '_loss'] = logging_outputs[0][
                        src_lang + '-' + tgt_lang + '_loss']
                    LS_valid_logging_output[src_lang + '-' + tgt_lang + '_size'] = logging_outputs[0][
        ...
        if getattr(cfg.task, "RS_epoch", None) and (sample is not None and len(sample) > 0):
            for lang_pair in cfg.model.reaction_pairs.split(','):
                src_lang, tgt_lang = lang_pair.split('-')
                if len(src_langs) == 1:  # O2M
                    logging_output[src_lang + '-' + tgt_lang + '_loss'] = LS_valid_logging_output[
                        src_lang + '-' + tgt_lang + '_loss']
                    logging_output[src_lang + '-' + tgt_lang + '_size'] = LS_valid_logging_output[
                        src_lang + '-' + tgt_lang + '_size']
                elif len(tgt_langs) == 1:  # M2O
                    logging_output[src_lang + '-' + tgt_lang + '_loss'] = LS_valid_logging_output[
                        src_lang + '-' + tgt_lang + '_loss']
        ...
        # 返回了但是没有被接收
        return logging_output, _loss, sample_size
    ...
    def _init_ema_model(self):
        ...
        if self._ema_device.type == "cpu":
            self._ema_model = self._ema_model.cpu()
        else:
            self._ema_model = self._ema_model.to(self._ema_device)

        # Convert to FP32 if requested
        if self._ema_fp32:
            self._ema_model = self._ema_model.float()

        # Set EMA model to eval mode
        self._ema_model.eval()

        logger.info(f"Initialized EMA model on {self._ema_device}")
    ...
    def update_ema(self):
        ...
        ema_params = dict(self._ema_model.named_parameters())
        ...
        with torch.no_grad():
            for name, param in model_params.items():
                if name in ema_params:
                    ...
                    if self._ema_fp32:
                        # Ensure both are in FP32
                        param_fp32 = param.float()
                        ema_param_fp32 = ema_param.float()
                        ema_param_fp32.mul_(self._ema_decay).add_(param_fp32, alpha=1.0 - self._ema_decay)
                        ema_param.copy_(ema_param_fp32)
                    else:
                        ema_param.mul_(self._ema_decay).add_(param, alpha=1.0 - self._ema_decay)
    ...
    def set_ema_decay(self, decay):
        ...
        logger.info(f"Updated EMA decay to {decay}")

    def load_ema_state_dict(self, state_dict):
        """Load EMA model from state dict."""
        if self._ema_model is None:
            self._init_ema_model()
        self._ema_model.load_state_dict(state_dict)
    ...
    def get_ema_state_dict(self):
        ...
        if self._ema_model is None:
            return None
        return self._ema_model.state_dict()
