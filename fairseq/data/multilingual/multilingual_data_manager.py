...
class MultilingualDatasetManager(object):
    def __init__(self, args, reaction_pairs, langs, dicts, sampling_method, pure_batch=False):
        super().__init__()
        ...
        self.extra_reaction_pairs = (
                list(
                    {p for _, v in args.extra_reaction_pairs.items() for p in v.split(",")}
                )
                if args.extra_reaction_pairs
                else []
            )
        self.src_langs = {p.split("-")[0] for p in args.reaction_pairs + self.extra_reaction_pairs}
        self.tgt_langs = {p.split("-")[1] for p in args.reaction_pairs + self.extra_reaction_pairs}
        self.langs = langs
        self.dicts = dicts
        self.reaction_dict = self.create_reaction_dictionary(self.langs)
    ...
    @classmethod
    def setup_data_manager(cls, args, reaction_pairs, langs, dicts, sampling_method, pure_batch=False):
        return MultilingualDatasetManager(
            args, reaction_pairs, langs, dicts, sampling_method, pure_batch=pure_batch
        )
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=None,
            type=csv_str_list,
            help="a list of languages comma sperated languages which can appear in reaction-pairs; "
        ...
        parser.add_argument(
            ...
            "languages which can appear in reaction-pairs; "
            "note that the ordering determines language token IDs; "
            "--langs and --reaction-dict are two exclusive options",
        )
        ...
        parser.add_argument(
            "--extra-reaction-pairs",
            help='a dictionary of data name to the language pairs they serve, \
                            e.g. {"mined": comma-separated-reaction-pairs, "denoised":  comma-separated-reaction-pairs}',
            type=lambda uf: eval_str_dict(uf, type=str),
            default=None,
        )
        parser.add_argument(
    ...
    def load_langs(cls, args, **kwargs):
        if args.reaction_dict and args.langs:
            raise ValueError("--langs and --reaction-dict can not both be specified")
        if args.reaction_dict is None and args.langs is None:
            logger.warning(
                "External language dictionary is not provided; "
                "use reaction-pairs to infer the set of supported languages. "
                "The language ordering is not stable which might cause "
                "misalignment in pretraining and finetuning."
            )
            # infer from reaction_pairs as it is
            langs = list(
                {x for lang_pair in args.reaction_pairs for x in lang_pair.split("-")}
            )
            langs = sorted(langs)
            logger.info(f"inferred language list: {langs}")
        elif args.reaction_dict:
            with open(
                PathManager.get_local_path(args.reaction_dict), "r", encoding="utf-8"
            ) as f:
                langs = [lang.strip() for lang in f.readlines() if lang.strip()]
                logger.info(
                    f"loaded language list from {args.reaction_dict} as they are ordered in file"
                )
        elif args.langs:
            langs = args.langs
            logger.info(
    ...
    def prepare(cls, load_dictionary, args, **kargs):
        ...
        if args.reaction_pairs is None:
            raise ValueError(
                "--reaction-pairs is required. List all the language pairs in the training objective."
            )
        if isinstance(args.reaction_pairs, str):
            args.reaction_pairs = args.reaction_pairs.split(",")
        ...
        check_langs(
            language_list,
            (
                [p.split("-") for p in args.reaction_pairs]
                if training
    ...
    def load_all_dictionaries(cls, args, language_list, load_dictionary, training):
        ...
        if training:
            ...
            src_langs_to_load_dicts = sorted(
                {p.split("-")[0] for p in (args.reaction_pairs + extra_reaction_pairs)}
            )
            tgt_langs_to_load_dicts = sorted(
                {p.split("-")[1] for p in (args.reaction_pairs + extra_reaction_pairs)}
            )
        else:
    ...
    @classmethod
    def create_reaction_dictionary(cls, langs):
        unk = "<unk>"
        # hack to remove symbols other than unk as they are not needed by lang dict
        reaction_dict = Dictionary(pad=unk, eos=unk, unk=unk, bos=unk)
        for lang in langs:
            reaction_dict.add_symbol(lang)
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        reaction_dictionary = self.reaction_dict
        ...
        langpair_ds = self.load_langpair_dataset(
            ...
            src_lang_id=_lang_id(reaction_dictionary, src)
            if enable_lang_ids and reaction_dictionary is not None
            else None,
            tgt_lang_id=_lang_id(reaction_dictionary, tgt)
            if enable_lang_ids and reaction_dictionary is not None
            else None,
    ...
    def get_data_paths_and_reaction_pairs(self, split):
        datapaths = {"main": self.args.data}
        reaction_pairs = {"main": self.reaction_pairs}
        if split == getattr(self.args, "train_subset", None):
            ...
            if self.args.extra_reaction_pairs:
                extra_reaction_pairs = {
                    k: v.split(",") for k, v in self.args.extra_reaction_pairs.items()
                }
                reaction_pairs.update(extra_reaction_pairs)
        return datapaths, reaction_pairs
    ...
    def get_split_num_data_shards(self, split):
        ...
        num_shards_dict = {}
        data_paths, reaction_pairs = self.get_data_paths_and_reaction_pairs(split)

        for data_category, paths in data_paths.items():
            if data_category not in reaction_pairs:
                continue
            paths = utils.split_paths(paths)
            shards_dict = self._get_shard_num_dict(split, paths)
            lang_dirs = [
                lang_pair.split("-") for lang_pair in reaction_pairs[data_category]
            ]
    ...
    def get_sampling_ratios(self, data_param_list, datasets, epoch, shard_epoch=None):
        if self.args.sampling_weights_from_file:  # False
        ...
        elif self.args.sampling_weights:  # False
    ...
    def load_into_concat_dataset(self, split, datasets, data_param_list, pure_batch=False):

        # print(f"DEBUG | lang_tok_replacing_bos_eos is {self.args.lang_tok_replacing_bos_eos}")
        if self.args.lang_tok_replacing_bos_eos:
            ...
            return SampledMultiDataset(
                ...
                virtual_size=None,
                split=split,
                pure_batch=pure_batch
            )
        return ConcatDataset([d for _, d in datasets], pure_batch=pure_batch)

    def load_sampled_multi_epoch_dataset(
        self, split, training, epoch=0, combine=False, shard_epoch=None, **kwargs
    ):
        """
            加载数据集并且返回一个SampledMultiDataset
        """
    ...
    def load_sampled_multi_dataset(
        ...
        # datasets: a list of fairseq.data.language_pair_dataset.LanguagePairDataset
        if training and split == getattr(self.args, "train_subset", None):
            sample_ratios = self.get_sampling_ratios(data_param_list, datasets, epoch)
            return SampledMultiDataset(
                OrderedDict(datasets),
                ...
                sampling_ratios=sample_ratios,
                eval_key=None,
                collate_format=CollateFormat.single,  # Modified by Eachan
                virtual_size=self.args.virtual_data_size,
                split=split,
                # if not using lang_tok altering, simplified to use the same collater
                shared_collater=self._shared_collater(),
                pure_batch=self.pure_batch
            )
        else:
            # print(f"DEBUG | when load in concat dataset, the pure-batch is {self.pure_batch}")
            return self.load_into_concat_dataset(split, datasets, data_param_list, pure_batch=self.pure_batch)

    def load_dataset(
        self, split, training, epoch=0, combine=False, shard_epoch=None, **kwargs
    ):
        # print(f"DEBUG | when split is {split}, the 'self.args.virtual_epoch_size is None' is {self.args.virtual_epoch_size is None}")
        if self.args.virtual_epoch_size is None:  # 走这个
