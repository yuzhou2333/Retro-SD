...
path_2_data=/root/autodl-tmp/Retro-SD/data_bin/usptofull_aug5_o2m
reaction_list=${path_2_data}/reaction_list.txt

checkpoint_path="/root/autodl-tmp/Retro-SD/checkpoints/usptofull_aug5_o2m/Retro_SD"
checkpoint_name=checkpoint157.pt
translate_path="/root/autodl-tmp/LSSD/results/usptofull/157"
if [ -n "$2" ]; then
  checkpoint_name=$2
fi
...
python ${project_path}/PCL_scripts/usptofull_aug5_o2m/result_statistics.py $TRANSLATION_DIR > ${TRANSLATION_DIR}/result.txt 2>&1