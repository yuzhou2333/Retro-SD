...
# 控制是否启用wandb
# 修改USE_WANDB变量为true/false来启用/禁用wandb日志记录
# 当USE_WANDB=true时，需要配置下面的WANDB相关环境变量
...
SAVE_DIR=${project_path}/checkpoints/usptofull_aug5_o2m/Retro_SD
mkdir -vp $SAVE_DIR
...
${python_path} ${project_path}/fairseq_cli/train.py $path_2_data \
  ...
  --save-interval 1 \
  --seed 11 --log-format simple --log-interval 10 \
  --bpe sentencepiece \
  --fp16 \
  --pure-batch \
  --RS-epoch \
...
bash ${project_path}/PCL_scripts/usptofull_aug5_o2m/test.sh $SAVE_DIR
