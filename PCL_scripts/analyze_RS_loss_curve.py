...
run example:
    python analyze_LS_loss_curve.py RS_valid_loss_history.json
"""
...
if __name__ == "__main__":
    ...
    reaction_pairs = list(data.keys())
    reaction_pairs.remove("all")
    reaction_pairs.append("all")
    lang_pair_num = len(reaction_pairs)
    ...
    for value, func in [("Convergence Epoch", get_convergence_epochs),
        ...
        print('-' * char_line)
        print('|' + '|'.join([f"{lang_pair:^{char_interval}}" for lang_pair in reaction_pairs]) + '|')
        print('-' * char_line)

        if value == "Distance to Upper-Bound":
            results = [func(data[lang_pair], get_convergence_epochs(data['all'])) for lang_pair in reaction_pairs]
            results[-1] = round(sum(results[:-1]), 3)
            print('|' + '|'.join([f"{i:^{char_interval}}" for i in results]) + '|')
        else:
            print('|' + '|'.join([f"{func(data[lang_pair]):^{char_interval}}" for lang_pair in reaction_pairs]) + '|')
